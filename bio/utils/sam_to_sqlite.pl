...
sub run {
	...
	warn "Creating table " . $self->table . "\n" if $self->verbose;
	{
		...
		$dbh->do(
			'CREATE TABLE '.$self->table.' ('.
				...
				'best_hit UNSIGNED INT(5),'.
				'number_of_best_hits UNSIGNED INT(5),'.
	...
	my $insert_statement = $dbh->prepare(
		q{INSERT INTO } . $self->table . q{ (id, qname, flag, rname, pos, mapq, cigar, rnext, pnext, tlen, seq, qual, tags, strand, start, stop, copy_number, sequence, mdz, number_of_mappings, best_hit, number_of_best_hits, query_length, alignment_length) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}
	);
	while (my $r = $sam->next_record) {
		...
		my $best_hit = $r->best_hit;
		if (!defined $best_hit) {
			$best_hit = 0;
		}

		my $number_of_best_hits = $r->number_of_best_hits;
		if (!defined $number_of_best_hits) {
			$number_of_best_hits = 0;
		}
		...
		$insert_statement->execute(undef, $r->qname, $r->flag, $r->rname,
			...
			$number_of_mappings, $best_hit, $number_of_best_hits, $r->query_length, $r->alignment_length);
