...
tee <(tr -cs A-Za-z '\n' | tr A-Z a-z | grep -vwFf "$WEB_INDEX_DIR/stopwords.txt" | "$WEB_INDEX_DIR/stem-words.js" | sort | uniq -c | sort -rn > 1-grams.txt) \
    <(tr -cs A-Za-z '\n' | tr A-Z a-z | bigrams_aux | sort | uniq -c | sort -rn > 2-grams.txt) \
    <(tr -cs A-Za-z '\n' | tr A-Z a-z | trigrams_aux | sort | uniq -c | sort -rn > 3-grams.txt) \
    > /dev/null
