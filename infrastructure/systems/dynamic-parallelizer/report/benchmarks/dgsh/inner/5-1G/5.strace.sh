...
strace -y -f --seccomp-bpf --trace=fork,clone,%file -o $logfile env -i cat "$INPUT_FILE" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z]/\n/g' | grep -v '^$' | sort | uniq | grep -v '^$' > "$file1"

# Ensure dictionary is compatibly sorted
logfile=$(generate_unique_file)
strace -y -f --seccomp-bpf --trace=fork,clone,%file -o $logfile env -i sort /usr/share/dict/words > "$file2"

# List errors as a set difference
logfile=$(generate_unique_file)
strace -y -f --seccomp-bpf --trace=fork,clone,%file -o $logfile env -i comm -23 "$file1" "$file2" 
